...
export default function Modal({ title=undefined, message=undefined, modalOpen=false, setModalOpen, btnAction}) {
  const cancelButtonRef = useRef(null)

  return (
    <Transition.Root show={modalOpen} as={Fragment}>
      <Dialog as="div" className="fixed z-10 inset-0 overflow-y-auto" initialFocus={cancelButtonRef} onClose={setModalOpen}>
              ...
              <div className="sm:flex sm:items-start">
                ...
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  <Dialog.Title as="h3" className="text-lg leading-6 font-medium text-gray-900">
                    {title || "Confirma o envio para: douglasprado@gmail.com" }
                  </Dialog.Title>
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      { message || "Enviaremos um e-mail para o e-mail do parceiro onde ele irá realizar o pagamento através de um link."}
                    </p>
                  </div>
                </div>
              </div>
