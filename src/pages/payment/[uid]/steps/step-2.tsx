...
import FormCardCredit from '@/components/Pages/Payments/FormCardCredit';

const steps = [
  { id: '01', name: 'Dados pessoais', href: '/payment/2/steps/step-1', status: 'complete' },
  { id: '02', name: 'Pagamento', href: '/payment/2/steps/step-2', status: 'current' },
  { id: '03', name: 'Confirmação', href: '/payment/2/steps/step-3', status: 'upcoming' },
]

export default function StepsTwo({ dataPayment, product }) {
  return (
    <div className="bg-white">
      ...
      <main className="relative grid grid-cols-1 gap-x-16 max-w-7xl mx-auto lg:px-8 lg:grid-cols-2 ">
        ...
        >
          <Steps steps={steps} />
          <FormCardCredit dataPayment={dataPayment}/>
        </section>
      </main>
    </div>
  )
...
export async function getServerSideProps(context) {
  ...
  if (!dataPayment) {
    return {
      redirect: {
        destination: `/payment/${uid}/steps/step-1`,
        permanent: false,
      },
    }
  }
